...
if [ "$1" = "main" ]; then
    npx completium-cli deploy contracts/battlemaster.arl --parameters '{"battle_master_fighter_id":6}'
else
    npx completium-cli deploy contracts/battlemaster.arl --parameters '{"battle_master_fighter_id":6}' --force
fi

# Step 4: Retrieve the contract address
CONTRACT_ADDRESS=$(npx completium-cli show address battlemaster | grep KT1)
